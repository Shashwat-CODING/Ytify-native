...
export default async function() {
  ...
  // Set the custom API URL to localhost:9999
  store.player.customAPI = 'http://localhost:9999';

  const custom_instance = getSaved('custom_instance');

  if (custom_instance) {
    // We still set these values for compatibility with other parts of the app
    // that might expect them, but we won't use them for stream fetching
    const [pi, iv, useInvidious] = custom_instance.split(',');
    store.player.hls.api[0] = store.api.piped[0] = pi;
    ...
    .then(res => res.json())
    .then(data => {
      // Initialize these for compatibility, but we'll override them with our custom API
