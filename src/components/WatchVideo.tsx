    ...
    >
      <video
        ...
        onerror={() => {
          ...
          fetch(`${store.player.customAPI}/streams/${store.actionsMenu.id}`)
            .then(res => res.json())
            .then(data => {
              if (data && data.streamingData && data.streamingData.adaptiveFormats) {
                const videoStreams = data.streamingData.adaptiveFormats
            ...
            .catch(() => {
              // Original fallback logic if needed
